...
- [ ] **Rediseño de núcleo (requiere decisión/dato, NO hecho)**: multi-jornada (revierte NO-GO 0114; decidir semántica de jornada activa + asignación entre jornadas), ciclo reservado→en_proceso de componentes (definir evento "inicio de trabajo"), consumo de MP en CNC/Pino (falta inventario de placas + listones reales), capacidad en "sets" (falta equivalencia unidad→set), reglas demorado/reprogramado (dependen del importador ML).
