...
- [x] **Accesibilidad mobile + auditoría BOM [2026-07-22]** (commit local, sin push) — ver HANDOFF. `BottomBar` mobile corregido (`produccion-hub`): los operarios ya alcanzan su sector desde el celular; carpinteria/logistica conservan legacy con flag ON. BOM real auditada read-only: **26/26 productos con BOM completa, 0 gaps** (`MAD030` OK). Validado: nav mobile real 8/8, gate web sin regresión, install 137/137, kill-switch 11/11. **Sin cambios en migraciones/remoto/flag.**
- [x] **Fase 8 multi-jornada BACKEND [2026-07-22]** (commit local, sin push, `0138` SOLO local) — hasta 3 abiertas configurable, 1 `en_ejecucion`, planificar/ejecutar/pausar, aislamiento de pedidos, producción a la activa. Backward-compatible. Kill-switch ampliado a **22 RPC** con test exhaustivo 33/33. Validado: install 138/138 ×3, upgrade 0135→0138 PASS, regresión 42/42, races 60/60, gaps 12/12, multi-jornada 18/18. **Falta la UI de jornadas.**
- [x] **Núcleo LP construido y CERTIFICADO [2026-07-22]** (`0139`–`0144` SOLO locales, sin push) — materia prima placas/Pino con consumo y bloqueo por config faltante; ciclo reservado→en_proceso con "Iniciar tarea" (columnas ya no en cero); capacidad configurable con `sets_equiv` nullable + override auditado; estados demorado/reprogramado + mapeo configurable (fail-closed); mínimos extendidos con dedup/reactivación; fix de vista multi-jornada. **UI de jornadas** (planificar/ejecutar/pausar/capacidad/override) web+mobile + data layer completo. Validado: install 144/144 ×3, upgrade 0135→0143 PASS, regresión 42/42, multi-jornada 18/18, tareas 14/14, cap/est/mín 18/18, **día de fábrica 21/21**, races 60/60, kill-switch 33 RPC 45/45, JornadasPanel + nav mobile PASS.
- [x] **RECONCILIACIÓN multijornada + coordinación reserva↔directa + auditoría [2026-07-22]** (`0146`/`0147` SOLO locales, sin push) — LP deja de tener un multijornada paralelo: **consume el sistema `jornadas` existente** (mismo id, misma `is_active`, misma asociación `orders.jornada_id`, tope de 3 controlado UNA vez por `rpc_open_jornada`). `prod_jornada` es un **espejo 1:1** por trigger; las RPC de jornada de LP **delegan** en `rpc_open_/set_active_/close_jornada`. Cierre de auditoría adversarial (5 hallazgos): RLS de escritura a `prod_jornada` eliminada (solo lectura), doble-membresía imposible (FK único), `reservar` sólo sobre la en_ejecucion, cierre libera reservas, `crear_solicitud` usa la activa. `0146`: carga directa CNC/Pino absorbe reservas (una sola contabilidad). Validado: install **147/147 ×13**, upgrade 0135→0147 PASS, **batería 12 suites VERDE** (regresión 42 · races PASS · gaps 12 · kill-switch 11+45 · multijornada reconciliada 20 · tareas 14 · cap/est/mín 16 · fábrica 21 · audit-fixes 12 · coordina 6 · **reconciliación 17**), **33 RPC mutantes** con kill-switch.
- [x] **Pantallas operativas LP construidas y montadas [2026-07-22]** (web+mobile, sin push) — tab **"Tareas LP"** (`LineaTareasPage`: ciclo reservado→en_proceso→finalizada, iniciar/finalizar parcial, cancelar, permisos por sector) y tab **"Configurar LP"** (`LineaConfigPage`: ABM materia prima, ajustes, equivalencias a sets, mínimos + alertas, estados externos) registradas en `produccion-hub` con gateo fail-closed por flag; selector `JornadasPanel` reconciliado (lee `prod_v_jornadas`, delega, capacidad informativa). 10/10 componentes LP transpilan (Babel classic).
- [ ] **Pre-deploy (dueño)**: aplicar `0136`/`0137` en remoto (backup confirmado) → redeploy EasyPanel → smoke URL pública (SW v15, tabs ocultas con flag OFF, CUIL→DNI) → autorización separada para encender flag `linea_productiva` + validación en fábrica.
...
- [x] **Marketing — CONSTRUIDA COMPLETA (2026-06-17).** Cockpit futurista con 5 módulos: Dashboard · Calendario (grilla mensual) · Ángulos de venta (drill-down ángulo→video→métricas) · Publicidad (CPM/CPC/CTR/CPR/ROAS) · Prioridades (con notificación). Backend 0090-0094 (7 tablas, 3 vistas, 14 RPCs, API-ready). Acceso por rol `marketing`/admin/owner.
  - **Cuenta del encargado:** `marketing@justomakario.app` / `Makario2026` (rol `marketing`). **RESTRINGIDA (2026-06-21):** su `ROLE_NAV` = `['marketing','notificaciones','perfil']` → **solo ve Marketing** (+ notificaciones donde caen las prioridades, + su perfil). Landing directo en Marketing. owner/admin siguen viendo todo.
