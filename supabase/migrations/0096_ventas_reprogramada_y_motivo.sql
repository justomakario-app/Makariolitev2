-- ════════════════════════════════════════════════════════════════════
-- VENTAS — Estados ML · FASE B: reprogramada (demorado) + motivo cancelación
-- ════════════════════════════════════════════════════════════════════
-- Spec especificacion_tecnica_v2 §4/§5. Decisiones cerradas por el Jefe:
--   · reprogramada = FLAG (no estado nuevo): la orden sigue 'pendiente' /
--     en producción, pero marcada. Distinto de 'arrastrado' (carry-over interno).
--   · detección por substring 'demorado' (no texto exacto).
--   · motivo de cancelación = 'Descripción del estado' del Excel (lo manda el parser).
-- 100% aditivo: columnas nuevas nullable + CREATE OR REPLACE del RPC
-- preservando TODA la lógica existente (cancelación, idempotencia, canal).
-- ════════════════════════════════════════════════════════════════════

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS reprogramada_at          timestamptz,
  ADD COLUMN IF NOT EXISTS reprogramada_motivo      text,
  ADD COLUMN IF NOT EXISTS reprogramada_in_jornada_id uuid,
...
DECLARE
  ...
  v_descripcion text; v_reprogramada int := 0; v_reprog_new boolean;   -- Fase B
BEGIN
  ...
  LOOP
    ...
    v_descripcion := NULLIF(trim(COALESCE(v_item->>'descripcion', '')), '');   -- Fase B
    v_sku := v_item->>'sku';
      ...
      IF NOT FOUND THEN
        INSERT INTO public.orders
          (channel_id, order_number, cliente, sku, cantidad, fecha_pedido,
           status, import_batch_id, jornada_id, cancelled_at, cancelled_in_jornada_id, cancelacion_motivo)
        VALUES (p_channel_id, v_order_number, v_item->>'cliente', v_sku, v_cantidad,
          COALESCE((v_item->>'fecha_pedido')::date, current_date),
          'cancelado', v_batch_id, v_jornada_id, now(), v_jornada_id, v_descripcion)
      ...
      ELSE
        ...
        UPDATE public.orders
          SET status = 'cancelado', cancelled_at = now(), cancelled_in_jornada_id = v_jornada_id,
              cancelacion_motivo = COALESCE(v_descripcion, cancelacion_motivo)
    ...
    -- ── REPROGRAMADA (contiene 'demorado') · FLAG, sigue activa ──
    ELSIF lower(trim(v_estado)) LIKE '%demorado%' THEN
      v_reprog_new := NULL;
      INSERT INTO public.orders
        (channel_id, order_number, cliente, sku, cantidad, fecha_pedido,
         status, import_batch_id, jornada_id, reprogramada_at, reprogramada_motivo, reprogramada_in_jornada_id)
      VALUES (p_channel_id, v_order_number, v_item->>'cliente', v_sku, v_cantidad,
        COALESCE((v_item->>'fecha_pedido')::date, current_date),
        'pendiente', v_batch_id, v_jornada_id, now(), v_estado, v_jornada_id)
...
-- ════════════════════════════════════════════════════════════════════
-- ROLLBACK (manual): re-aplicar la versión previa del RPC (sin la rama
-- 'demorado' ni cancelacion_motivo) y DROP COLUMN reprogramada_at,
-- reprogramada_motivo, reprogramada_in_jornada_id, cancelacion_motivo.
-- ════════════════════════════════════════════════════════════════════
