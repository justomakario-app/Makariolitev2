-- ════════════════════════════════════════════════════════════════════
-- ETAPA — RPCs para CARGA MANUAL + EDICIÓN DE PEDIDOS
-- ════════════════════════════════════════════════════════════════════
-- Sub-etapa 2.2 del plan aprobado.
--
-- 1) rpc_create_manual_order — carrito atómico, valida jornada activa,
--    autogenera order_number si no se da, soporta force_merge para
--    colisión deliberada con order existente.
--
-- 2) rpc_edit_order — edita items existentes con optimistic locking,
--    quita items, agrega items nuevos. Atomicidad obligatoria con
--    INSERTs en order_edit_log: si el log falla, rollback total.
--
-- Permisos: owner, admin, encargado. Operario rechazado.
-- Distribuidor: out_of_scope.
-- ════════════════════════════════════════════════════════════════════

-- ════════════════════════════════════════════════════════════════════
-- rpc_create_manual_order
-- ════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF v_role NOT IN ('owner','admin','encargado') THEN
    RAISE EXCEPTION 'No tenes permiso para cargar pedidos manuales.'
  ...
  IF p_channel_id = 'distribuidor' THEN
    RAISE EXCEPTION 'Distribuidor esta fuera del scope de carga manual.'
      USING ERRCODE='22023', HINT='out_of_scope';
  END IF;

  -- ── Items array ──
  IF jsonb_typeof(p_items) <> 'array' OR jsonb_array_length(p_items) = 0 THEN
    RAISE EXCEPTION 'Tenes que agregar al menos un item.'
      USING ERRCODE='22023', HINT='no_items';
  END IF;

  -- Pre-validar SKUs (todos al inicio para mensaje claro)
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    v_sku := upper(trim(v_item->>'sku'));
    v_cantidad := COALESCE((v_item->>'cantidad')::int, 0);
    IF v_sku IS NULL OR v_sku = '' THEN
      RAISE EXCEPTION 'Hay un item sin SKU.'
        USING ERRCODE='22023', HINT='empty_sku';
    END IF;
    IF v_cantidad <= 0 THEN
      RAISE EXCEPTION 'La cantidad de % debe ser mayor a 0 (recibido %).', v_sku, v_cantidad
        USING ERRCODE='22023', HINT='invalid_qty';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM public.sku_catalog WHERE sku = v_sku AND activo = true) THEN
      v_unknown_skus := array_append(v_unknown_skus, v_sku);
    END IF;
  END LOOP;
  IF array_length(v_unknown_skus, 1) > 0 THEN
    RAISE EXCEPTION 'SKU(s) desconocido(s) o inactivo(s): %', array_to_string(v_unknown_skus, ', ')
      USING ERRCODE='23503', HINT='sku_not_found';
  END IF;

  -- ── Jornada activa ──
  SELECT id, fecha INTO v_jornada_id, v_jornada_fecha
    FROM public.jornadas
    WHERE channel_id = p_channel_id AND status = 'abierta' AND is_active = true
    LIMIT 1;
  IF v_jornada_id IS NULL THEN
    RAISE EXCEPTION 'No hay jornada activa para %. Pedile al encargado que abra una.', v_channel_label
      USING ERRCODE='22023', HINT='no_active_jornada';
  END IF;

  -- ── Resolver order_number ──
  IF p_order_number IS NULL OR trim(p_order_number) = '' THEN
    -- Autogenerar MAN-YYYYMMDD-NNNN. NNNN = contador del dia con ese prefijo.
    v_today := to_char(current_date, 'YYYYMMDD');
    SELECT COUNT(*) INTO v_seq
      FROM public.orders
      WHERE order_number LIKE 'MAN-' || v_today || '-%';
    v_order_number := 'MAN-' || v_today || '-' || lpad((v_seq + 1)::text, 4, '0');
  ELSE
    ...
    IF v_existing_count > 0 AND p_force_merge = false THEN
      -- Reportar colisión sin insertar nada
      RAISE EXCEPTION 'El numero % ya existe en % con % items.', v_order_number, v_channel_label, v_existing_count
        USING ERRCODE='22023', HINT='order_number_exists';
    END IF;
  END IF;

  -- ── INSERT items (transacción atómica de plpgsql) ──
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    v_sku := upper(trim(v_item->>'sku'));
    ...
    BEGIN
      ...
      VALUES (
        p_channel_id, v_order_number,
        NULLIF(trim(coalesce(p_cliente, '')), ''),
        v_sku, v_cantidad,
        current_date, 'pendiente', v_jornada_id, 'manual', 1, auth.uid()
      );
    EXCEPTION WHEN unique_violation THEN
      -- En force_merge, si el item exacto (channel+order_number+sku) ya existe,
      -- es duplicado real → reject para que el admin sume la cantidad via edit.
      RAISE EXCEPTION 'El item % ya existe en pedido %. Si querias sumar, usa Editar pedido.', v_sku, v_order_number
        USING ERRCODE='23505', HINT='item_duplicate';
    END;

    v_inserted := v_inserted + 1;
    v_total_units := v_total_units + v_cantidad;
    v_items_out := v_items_out || jsonb_build_object('sku', v_sku, 'cantidad', v_cantidad, 'version', 1);
  END LOOP;

  -- ── Notificación a owner+encargado (excepto creador) ──
  INSERT INTO public.notifications (user_id, tipo, titulo, mensaje, link)
  SELECT p.id, 'nuevo_pedido',
    format('Nuevo pedido manual en %s', v_channel_label),
    format('%s cargo pedido #%s: %s items, %s uds total',
      v_user_name, v_order_number, v_inserted, v_total_units),
  ...
  RETURN jsonb_build_object(
    ...
    'jornada_fecha',  v_jornada_fecha,
    'items_created',  v_inserted,
    'total_units',    v_total_units,
    'items',          v_items_out,
    'merged_existing', (v_existing_count > 0)
  );
END;
$func$;

REVOKE ALL ON FUNCTION public.rpc_create_manual_order(text, text, text, jsonb, text, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.rpc_create_manual_order(text, text, text, jsonb, text, boolean) TO authenticated;

-- ════════════════════════════════════════════════════════════════════
-- rpc_edit_order
-- ════════════════════════════════════════════════════════════════════
-- p_cambios: jsonb objeto con tres arrays opcionales:
--   modificar: [{sku, cantidad_nueva, version}]
...
BEGIN
  ...
  ELSE
    -- Sin jornada asignada (Excel pre-cierre): usar la activa del canal
