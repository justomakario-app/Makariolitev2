-- ════════════════════════════════════════════════════════════════════
-- 0176 — import_batches: los contadores que nunca se escribieron
-- ════════════════════════════════════════════════════════════════════
-- El sintoma: la tabla "Lotes importados" de cada canal muestra una columna "Pedidos" que
-- decia 0 en los 310 lotes de la base. Al lado tiene un boton "Eliminar" que borra el lote,
-- TODAS sus ordenes y los registros de produccion de esos SKU desde la fecha del lote, y el
-- cartel de confirmacion tomaba el mismo numero: "y TODAS sus ordenes (0 pedidos)" para un
-- lote con 186 pedidos reales. Una accion irreversible pidiendo permiso con el numero
-- equivocado.
--
-- La causa: rpc_import_batch inserta la fila de import_batches ARRIBA DE TODO, antes de
-- recorrer los items, para tener el v_batch_id que despues cuelga de cada orden. Nunca
-- vuelve a tocar esa fila, asi que las tres columnas de contadores quedan en su default.
--
-- Este archivo cambia UNA cosa: agrega el UPDATE de los contadores justo antes del RETURN,
-- con los valores que la funcion ya venia calculando y devolviendo en el jsonb. El resto
-- del cuerpo es copia literal de la 0099 (el archivo se genera copiando el texto, no
-- transcribiendolo, y tests/import-lote-test.js verifica que la unica diferencia entre las
-- dos versiones sea ese bloque).
--
-- Ademas backfillea los lotes viejos contando contra orders.
--
-- El front ya no depende de esto: cuenta contra `orders` en el momento de mostrar y en el
-- momento de borrar, que es la unica fuente que no puede desincronizarse. Esta migracion
-- arregla el dato guardado, que es lo que va a leer cualquier reporte futuro.
--
-- Depende de: 0099 (ultima version viva de rpc_import_batch).
-- Rollback al pie.
-- ════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- 0176: los contadores del lote. El INSERT de import_batches ocurre ARRIBA, antes de
  -- procesar un solo item, asi que pedidos_count y unidades_count nunca se escribian: los
  -- 310 lotes de la base quedaron en cero. La pantalla "Lotes importados" mostraba ese cero
  -- como si fuera el tamano del lote, y el cartel de borrado (que borra el lote Y todas sus
  -- ordenes Y los logs de produccion de esos SKU) decia "0 pedidos" sobre lotes de 186.
  --
  -- Solo estas dos columnas. `skus_desconocidos` es text[] (el array de SKU que el Excel
  -- traia y el catalogo no tiene) y la funcion nunca los junta: solo lleva el contador
  -- v_skipped_unknown. Llenarlo pide acumular los SKU dentro del loop, que es otro cambio
  -- y va aparte: no se mete SQL sin probar en el mismo archivo que arregla otra cosa.
...
-- ── Backfill de los 310 lotes que quedaron en cero ──────────────────────────────────────
-- Solo toca los que estan en cero: si alguno tuviera un numero cargado a mano, se respeta.
UPDATE public.import_batches b
   SET pedidos_count  = c.pedidos,
       unidades_count = c.unidades
  FROM (
    SELECT import_batch_id, count(*) AS pedidos, coalesce(sum(cantidad), 0) AS unidades
      FROM public.orders
     WHERE import_batch_id IS NOT NULL
     GROUP BY import_batch_id
  ) c
 WHERE b.id = c.import_batch_id
   AND coalesce(b.pedidos_count, 0) = 0
   AND coalesce(b.unidades_count, 0) = 0;

commit;

-- ════════════════════════════════════════════════════════════════════
-- ROLLBACK (manual): re-aplicar 0099. El backfill no se deshace (son
-- numeros correctos; volver a ponerlos en cero seria volver al bug).
-- ════════════════════════════════════════════════════════════════════
