...
Verde = `10/10 suites en verde · 498 checks ok · 0 fail`. Si algo sale en rojo,
...
| `tienda-render-test.js` | La tienda de `tienda/`: **alta abierta por link** (los dos pasos, el CUIT que valida y el que no, correo repetido, CUIT ya conocido → queda `pendiente`, y que el payload no lleve `role`, `token` ni `canal`), **elección de catálogo** (no pide precios antes de elegir, el cambio recarga cuenta **y** carrito, y con un solo canal se auto-elige), canje de invitación, **recuperar la contraseña** (que la respuesta no delate si el correo está registrado, que el token del mail se canjee por sesión y se borre de la barra de direcciones), carrito y alta de pedido. |
