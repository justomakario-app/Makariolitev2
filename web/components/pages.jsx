...
function CatalogoPage() {
  ...
  const onSave = async (sku, data, isNew) => {
    if (isNew && window.SKU_DB[sku]) { toast.error(`SKU ${sku} ya existe`); return false; }
    try {
      const payload = {
        modelo: data.modelo,
        color: data.color,
...
function EquipoPage() {
  ...
  const onSave = async (original, data, isNew) => {
    try {
      if (isNew) {
        ...
        toast.success(`${data.name} agregado al equipo · password: ${data.password}`);
      } else {
        await window.MOCK_ACTIONS.actualizarPerfil({
          id: original.id,
          data: { name: data.name, role: data.role, area: data.area },
        });
  ...
  const onToggleActive = async (u) => {
    try {
      await window.MOCK_ACTIONS.toggleProfileActive({ id: u.id, active: !u.active });
  ...
  const onRemove = async (u) => {
    if (!confirm(`¿Desactivar a ${u.name}? Podés reactivarlo después.`)) return;
    try {
      await window.MOCK_ACTIONS.toggleProfileActive({ id: u.id, active: false });
