    ...
    <div>
      ...
      <div className="card" style={{marginTop:16}}>
        ...
        <table className="data-table">
          <thead>
            <tr>
              <th>Email invitado</th><th>Cliente</th><th>Canal</th>
              <th>Emitida</th><th>Vence</th><th>Estado</th><th></th>
            </tr>
          </thead>
          <tbody>
            {invits.map(i => (
              <tr key={i.id}>
                ...
                <td>{vencida(i) ? badgeEstado('vencida') : badgeEstado(i.estado)}</td>
                <td className="cta-cte-actions">
                  {i.estado === 'pendiente' && (
                    <button className="btn-ghost-sm danger" disabled={revocando}
                            title={vencida(i)
                              ? 'Sacarla de la lista: ya no sirve'
                              : 'Dar de baja el código antes de que lo usen'}
