import { useEffect, useState } from 'react';
import { Modal } from '@/components/shared/Modal';
import { Icon } from '@/components/shared/Icon';
import { useToast } from '@/components/shared/Toast';
import { useChannels } from '@macario/shared/hooks/useChannels';
import { useSkuCatalog } from '@macario/shared/hooks/useCatalog';
import { useRegisterProduction } from '@macario/shared/hooks/useCarrier';

interface Props {
  open: boolean;
  defaultSku?: string;
  defaultChannelId?: string;
  onClose: () => void;
}

export function ProduceModal({ open, defaultSku, defaultChannelId, onClose }: Props) {
  const toast = useToast();
  const { data: skus = [] } = useSkuCatalog();
  const { data: channels = [] } = useChannels();
  const register = useRegisterProduction();

  const [step, setStep] = useState(1);
  const [sku, setSku] = useState(defaultSku ?? '');
  const [search, setSearch] = useState('');
  const [channelId, setChannelId] = useState(defaultChannelId ?? '');
  const [cantidad, setCantidad] = useState(1);
  const [notas, setNotas] = useState('');

  useEffect(() => {
    if (open) {
      const hasSku = !!defaultSku;
      const hasCh = !!defaultChannelId;
      setStep(hasSku && hasCh ? 3 : hasSku ? 2 : 1);
      setSku(defaultSku ?? '');
      setSearch('');
      setChannelId(defaultChannelId ?? '');
      setCantidad(1);
      setNotas('');
    }
  }, [open, defaultSku, defaultChannelId]);

  const filtered = skus.filter(s => {
    if (!search) return true;
    const q = search.toLowerCase();
    return s.sku.toLowerCase().includes(q)
      || s.modelo.toLowerCase().includes(q)
      || (s.color ?? '').toLowerCase().includes(q);
  });

  const skuInfo = skus.find(s => s.sku === sku);

  const submit = async () => {
    if (!sku || !channelId || cantidad < 1) return;
    try {
      await register.mutateAsync({
        sku, channel_id: channelId, cantidad, notas: notas.trim() || null,
      });
      const ch = channels.find(c => c.id === channelId);
      toast.success(`${cantidad} × ${sku} → ${ch?.label ?? channelId}`);
      onClose();
    } catch (e) {
      toast.error((e as Error).message);
    }
  };

  return (
    <Modal open={open} onClose={onClose} title="Registrar producción" size="lg" footer={
      <>
        {step === 3 ? (
          <button
            className="btn-primary"
            onClick={submit}
            disabled={register.isPending || cantidad < 1 || !sku || !channelId}
            style={{minHeight: 50}}
          >
            {register.isPending
              ? <span className="loader" style={{borderColor: 'rgba(255,255,255,.3)', borderTopColor: '#fff'}}/>
              : <><Icon n="check" s={16}/> Confirmar</>}
          </button>
        ) : (
          <button
            className="btn-primary"
            onClick={() => setStep(step + 1)}
            disabled={(step === 1 && !sku) || (step === 2 && !channelId)}
            style={{minHeight: 50}}
          >
            Siguiente <Icon n="arrow-right" s={14}/>
          </button>
        )}
        {step > 1 && (
          <button className="btn-ghost" onClick={() => setStep(step - 1)} style={{minHeight: 44}}>
            <Icon n="arrow-left" s={14}/> Atrás
          </button>
        )}
      </>
    }>
      {/* Stepper compacto */}
      <div style={{display: 'flex', alignItems: 'center', gap: 6, marginBottom: 18}}>
        {['Producto', 'Canal', 'Cantidad'].map((lbl, i) => {
          const n = i + 1;
          const active = step === n;
          const done = step > n;
          return (
            <div key={lbl} style={{display: 'flex', alignItems: 'center', flex: i === 2 ? 0 : 1, gap: 6}}>
              <div style={{
                width: 22, height: 22, borderRadius: '50%',
                background: done ? 'var(--green)' : active ? 'var(--ink)' : 'var(--paper-dim)',
                color: done || active ? '#fff' : 'var(--ink-muted)',
                fontSize: 10, fontWeight: 800,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                flexShrink: 0,
              }}>
                {done ? <Icon n="check" s={10}/> : n}
              </div>
              <div style={{
                fontSize: 10, fontWeight: 700,
                color: active || done ? 'var(--ink)' : 'var(--ink-muted)',
                textTransform: 'uppercase', letterSpacing: '.06em',
              }}>{lbl}</div>
              {i < 2 && <div style={{flex: 1, height: 1, background: done ? 'var(--green)' : 'var(--border)', margin: '0 4px'}}/>}
            </div>
          );
        })}
      </div>

      {/* Paso 1: SKU search + lista */}
      {step === 1 && (
        <div>
          <div style={{position: 'relative', marginBottom: 10}}>
            <input
              className="field-input"
              placeholder="Buscar SKU, modelo o color…"
              value={search}
              onChange={e => setSearch(e.target.value)}
              style={{paddingLeft: 38}}
              autoFocus
            />
            <span style={{position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', display: 'flex'}}>
              <Icon n="search" s={16} c="var(--ink-muted)"/>
            </span>
          </div>
          <div style={{
            maxHeight: '52vh', overflowY: 'auto',
            border: '1px solid var(--border)', borderRadius: 8,
            background: 'var(--paper-off)',
          }}>
            {filtered.length === 0 ? (
              <div style={{padding: 20, textAlign: 'center', fontSize: 12, color: 'var(--ink-muted)'}}>
                Sin resultados
              </div>
            ) : filtered.map(s => {
              const sel = sku === s.sku;
              return (
                <button
                  key={s.sku}
                  onClick={() => setSku(s.sku)}
                  style={{
                    display: 'flex', alignItems: 'center', gap: 10, width: '100%',
                    padding: '14px 14px', border: 'none',
                    borderLeft: sel ? '3px solid var(--ink)' : '3px solid transparent',
                    borderBottom: '1px solid var(--border)',
                    background: sel ? 'var(--ink)' : 'transparent',
                    color: sel ? '#fff' : 'var(--ink)',
                    cursor: 'pointer', textAlign: 'left',
                    minHeight: 56,
                  }}
                >
                  <span style={{
                    minWidth: 60,
                    fontFamily: 'var(--mono)',
                    fontSize: 11, fontWeight: 700,
                    color: sel ? 'rgba(255,255,255,.85)' : 'var(--ink-muted)',
                  }}>{s.sku}</span>
                  <div style={{flex: 1, minWidth: 0}}>
                    <div style={{fontSize: 13, fontWeight: 600}}>{s.modelo}</div>
                    {s.color && s.color !== '—' && (
                      <div style={{
                        fontSize: 11, marginTop: 1,
                        color: sel ? 'rgba(255,255,255,.7)' : 'var(--ink-muted)',
                      }}>
                        {s.color} · {s.categoria}
                      </div>
                    )}
                  </div>
                  {sel && <Icon n="check" s={16}/>}
                </button>
              );
            })}
          </div>
        </div>
      )}

      {/* Paso 2: canal */}
      {step === 2 && (
        <div>
          <div style={{display: 'flex', flexDirection: 'column', gap: 8}}>
            {channels.map(c => (
              <label
                key={c.id}
                style={{
                  display: 'flex', alignItems: 'center', gap: 12,
                  padding: 14,
                  border: `2px solid ${channelId === c.id ? c.color : 'var(--border)'}`,
                  background: channelId === c.id ? `${c.color}1a` : 'var(--paper)',
                  borderRadius: 10, cursor: 'pointer',
                  minHeight: 64,
                }}
              >
                <input
                  type="radio"
                  checked={channelId === c.id}
                  onChange={() => setChannelId(c.id)}
                  style={{width: 20, height: 20, accentColor: c.color}}
                />
                <div style={{flex: 1}}>
                  <div style={{fontSize: 14, fontWeight: 700, color: 'var(--ink)'}}>{c.label}</div>
                  <div style={{fontSize: 11, color: 'var(--ink-muted)', marginTop: 2}}>{c.sub ?? ''}</div>
                </div>
                {channelId === c.id && <Icon n="check-circle" s={20} c={c.color}/>}
              </label>
            ))}
          </div>
        </div>
      )}

      {/* Paso 3: cantidad + nota */}
      {step === 3 && (
        <div>
          <label className="field-label">Cantidad producida</label>
          <div style={{display: 'flex', gap: 8, alignItems: 'center', marginBottom: 14}}>
            <button
              onClick={() => setCantidad(Math.max(1, cantidad - 1))}
              className="btn-ghost"
              style={{padding: '14px 18px', fontSize: 22, lineHeight: 1, minWidth: 52, flexShrink: 0}}
            >−</button>
            <input
              type="number"
              inputMode="numeric"
              min="1"
              value={cantidad}
              onChange={e => setCantidad(Math.max(1, parseInt(e.target.value) || 1))}
              className="field-input"
              style={{textAlign: 'center', fontSize: 22, fontWeight: 800, fontFamily: 'var(--mono)', height: 56}}
            />
            <button
              onClick={() => setCantidad(cantidad + 1)}
              className="btn-ghost"
              style={{padding: '14px 18px', fontSize: 22, lineHeight: 1, minWidth: 52, flexShrink: 0}}
            >+</button>
          </div>

          <label className="field-label">Nota (opcional)</label>
          <input
            className="field-input"
            placeholder="Ej: lote especial"
            value={notas}
            onChange={e => setNotas(e.target.value)}
          />

          {/* Resumen */}
          <div style={{
            marginTop: 16, padding: 14,
            background: 'var(--paper-off)',
            border: '1px solid var(--border)', borderRadius: 8,
          }}>
            <div style={{fontSize: 9, fontWeight: 700, letterSpacing: '.14em', textTransform: 'uppercase', color: 'var(--ink-muted)', marginBottom: 8}}>
              Resumen
            </div>
            <div style={{fontSize: 13, lineHeight: 1.7}}>
              <div>
                <strong style={{fontFamily: 'var(--mono)', fontSize: 16}}>{cantidad}×</strong>
                {' '}{sku}
                {skuInfo && ` — ${skuInfo.modelo}${skuInfo.color && skuInfo.color !== '—' ? ' ' + skuInfo.color : ''}`}
              </div>
              <div style={{marginTop: 4}}>
                Canal: <strong>{channels.find(c => c.id === channelId)?.label}</strong>
              </div>
              <div style={{marginTop: 4, fontSize: 11, color: 'var(--ink-muted)'}}>
                Operario y sector se asignan auto desde tu sesión.
              </div>
            </div>
          </div>
        </div>
      )}
    </Modal>
  );
}
