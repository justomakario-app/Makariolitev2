/* ══ TIENDA MAYORISTA — CATÁLOGO Y PRECIOS (mobile)
   ...
   CARGA MASIVA: son 61 SKU y dos catálogos. "Pegar precios" acepta lo que
   sale de copiar dos columnas de Excel, y se elige a qué columna van: al
   precio de lista o directo a un canal. Quedan en el mismo borrador que la
   edición manual — se revisan en pantalla y recién después se guardan.

   ── Lo que cambia respecto del desktop ──
   La grilla no entra en un teléfono: se scrollea de costado dentro de la
   tarjeta y los campos son más angostos. Es a propósito y no se convierte
   en tarjetas apiladas — cargar precios es comparar columnas, y apilarlas
   obliga a memorizar el número de arriba para controlar el de abajo. El
   modal de reglas sí va a una sola columna, ahí no hay nada que comparar.
   ══ */
...
function B2BCatalogoTab({ isOwner } = {}) {
  ...
  const guardar = async () => {
    ...
    try {
      ...
      /* Sin este finally el botón quedaba en "Guardando…" para siempre cuando
         el guardado salía bien, y había que recargar la app para tocar otro
  ...
  return (
    <div>
      ...
      <div style={{display:'flex', flexWrap:'wrap', gap:'4px 14px', padding:'0 2px 6px',
                   fontSize:12, color:'var(--ink-muted)'}}>
        <span><b style={{color:'var(--ink)'}}>{items.length}</b> productos</span>
        <span><b style={{color:'#15803d'}}>{publicados}</b> publicados</span>
        {sinPrecio > 0 && <span><b style={{color:'#b45309'}}>{sinPrecio}</b> sin precio</span>}
        {conPropio > 0 && <span><b style={{color:'#b45309'}}>{conPropio}</b> con precio propio</span>}
      </div>
                ...
                {canalesActivos.map(c => (
                  <th key={c.codigo} style={{textAlign:'right', whiteSpace:'nowrap'}}>
                    ...
                    <div style={{fontWeight:400, fontSize:10, color:'var(--ink-muted)'}}>
                      ×{Number(c.coeficiente).toFixed(2)} · o propio
                    </div>
                  </th>
                ))}
                  ...
                  <tr key={it.sku} style={tocado ? {background:'#fffbeb'} : undefined}>
                    ...
                    <td style={{textAlign:'right'}}>
                      <input className={`field-input ${malo || cero ? 'has-error' : ''}`}
                             ...
                             style={{width:104, textAlign:'right', padding:'4px 8px', fontSize:13}}
      ...
      {nCambios > 0 && (
        <div style={{position:'sticky', bottom:0, marginTop:12, padding:'12px 14px',
          ...
          <button className="btn-ghost" style={{flex:1}}
                  onClick={() => setEdits({})} disabled={guardando}>Descartar</button>
...
/* ── Canales: coeficientes y mínimos ──────────────────────────────────
   ...
   Desde 0160 el coeficiente es el RESPALDO, no la única regla: los SKU con
   precio propio cargado no se mueven cuando esto cambia. Se dice acá para
   que nadie toque el coeficiente esperando repreciar el catálogo entero. ── */
  ...
  return (
    <div className="card" style={{marginBottom:14, padding:'14px'}}>
      ...
      <div style={{fontSize:12, color:'var(--ink-muted)', marginBottom:12, lineHeight:1.5}}>
        El coeficiente multiplica el precio de lista. Cambiarlo reprecia el
        catálogo de ese canal, salvo los productos que tengan precio propio
        cargado: esos quedan como están.
                ...
                <tr key={c.codigo}>
                  ...
                  <td style={{textAlign:'right'}}>
                    <input className={`field-input ${kMalo ? 'has-error' : ''}`}
                           type="number" inputMode="decimal" step="0.01" min={0.01} max={5}
                           disabled={!isOwner}
                           style={{width:92, textAlign:'right', padding:'4px 8px', fontSize:13}}
                           value={c.coeficiente}
                  ...
                  <td style={{textAlign:'right'}}>
                    <input className="field-input" type="number" inputMode="numeric" min={0} step={1}
                           disabled={!isOwner}
                           style={{width:92, textAlign:'right', padding:'4px 8px', fontSize:13}}
      ...
      {confirmar && (
        <window.ConfirmModal
          ...
          message="Esto cambia el precio de los productos publicados que NO tengan precio propio en esos canales. Los que tienen precio propio no se mueven, y los pedidos ya enviados tampoco: cada uno guarda el precio con el que se cerró. ¿Seguimos?"
...
/* ── Modal: pegar precios desde Excel ─────────────────────────────────
   ...
   después otra vez en la grilla con los precios por canal resueltos. ── */
  ...
  return (
    <Cmp open={true} title="Pegar precios" onClose={onClose} footer={
      ...
      <div className="field-group">
        ...
        <textarea className="field-input" rows={7} autoFocus
      ...
      {texto.trim() && (
        <div className="field-group">
          ...
          <div style={{marginTop:10, fontSize:11, color:'var(--ink-muted)', lineHeight:1.5}}>
            ...
            grilla, con lo que paga cada canal ya resuelto. Se revisa y recién
...
/* ── Ficha del producto: alta y edición ───────────────────────────────
   (mobile) Un solo modal para las dos cosas: lo que
    ...
    <Cmp open={true} size="lg"
      ...
      {esNuevo && (
        <div className="field-group">
          ...
          <div className="field-help">
            2 a 4 letras y 2 a 5 números. No se puede cambiar después.
          </div>
        </div>
      )}
      ...
      <div className="field-group">
        ...
        <div className="field-help">Lo ve el mayorista en la tienda.</div>
      </div>
      ...
      <div className="field-help" style={{marginBottom:4}}>
        Desactivado sale de todo el sistema y no aparece en la tienda,
      ...
      <div className="field-group">
        <label className="field-label">Precio de lista (sin IVA)</label>
        <input className="field-input" type="number" inputMode="decimal" min={0} step="0.01"
               value={f.precio_base} onChange={e => set('precio_base', e.target.value)}
               placeholder="sin precio"/>
        <div className="field-help">
          Base de cada canal. Hace falta para publicar.
        </div>
      </div>
      ...
      <div style={{fontSize:12, color:'var(--ink-muted)', marginBottom:12, lineHeight:1.5}}>
        Las controla el servidor al agregar al carrito y al enviar el pedido.
      </div>

      <div style={{display:'grid', gridTemplateColumns:'1fr', gap:12}}>
        <div className="field-group">
          <label className="field-label">Se vende de a (múltiplo)</label>
          <input className="field-input" type="number" inputMode="numeric" min={1} step={1}
                 value={f.multiplo_venta}
                 onChange={e => set('multiplo_venta', e.target.value === '' ? '' : Number(e.target.value))}/>
          <div className="field-help">Con 4: 4, 8, 12 — no 6.</div>
        </div>
        ...
        <div className="field-group">
          ...
          <div className="field-help">Informativo, para el remito.</div>
        </div>

        <div className="field-group">
          <label className="field-label">IVA %</label>
          <input className="field-input" type="number" inputMode="decimal" min={0} max={100} step="0.01"
                 value={f.iva_pct}
      ...
      <div className="field-group">
        ...
        <input className="field-input" type="text" maxLength={20}
               ...
               placeholder="unidad"/>
        <div className="field-help">unidad, juego, par, metro.</div>
      </div>
      ...
      <div className="field-group">
        ...
        <textarea className="field-input" rows={3} maxLength={400}
                  ...
                  placeholder="Opcional. Lo ve el mayorista."/>
      </div>
