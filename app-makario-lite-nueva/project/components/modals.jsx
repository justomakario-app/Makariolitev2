...
function ProduceModal({ open, onClose, defaultSku, defaultSubcanal }) {
  ...
  const submit = () => {
    setBusy(true);
    setTimeout(() => {
      window.MOCK_ACTIONS.registrarProduccion({ sku, subcanal, cantidad });
      setBusy(false);
      onClose();
...
function ImportModal({ open, onClose, channel: defaultChannel }) {
  ...
  const submit = () => {
    ...
    const t = setInterval(() => {
      ...
      if (p >= 100) {
        clearInterval(t);
        // Aplicar pedidos al canal seleccionado
        // Agrupar por SKU para reducir llamadas
        const porSku = {};
        let aplicados = 0; let ignorados = 0;
        for (const o of orders) {
          if (!window.SKU_DB[o.sku]) { ignorados += o.cantidad; continue; }
          porSku[o.sku] = (porSku[o.sku] || 0) + o.cantidad;
          aplicados += o.cantidad;
        }
        for (const [sku, cantidad] of Object.entries(porSku)) {
          window.MOCK_ACTIONS.aplicarPedido({ sku, subcanal: channel, cantidad });
        }
        // Registrar lote
        const carrier = window.MOCK.carriers[channel];
        if (carrier) {
          const ahora = new Date();
          const stamp = ahora.toISOString().slice(0,16).replace('T',' ');
          carrier.lotes.unshift({
            id: `L-${String(carrier.lotes.length+1).padStart(3,'0')}`,
            fecha: stamp,
            cantidad: aplicados,
            archivo: file.name,
            detectado: detected || window.CARRIERS[channel].label,
          });
          // Agregar pedidos a la lista de orders del carrier
          for (const o of orders) {
            if (!window.SKU_DB[o.sku]) continue;
            carrier.orders.unshift({
              numero: o.numero || `IMP-${Math.random().toString(36).slice(2,8).toUpperCase()}`,
              cliente: o.cliente || '—',
              sku: o.sku,
              cantidad: o.cantidad,
              fecha: (o.fecha || '').slice(0,10) || ahora.toISOString().slice(0,10),
            });
          }
        ...
        onClose();
        if (ignorados > 0) {
          toast.success(`${file.name} importado · ${aplicados} uds. aplicadas a ${window.CARRIERS[channel]?.label} · ${ignorados} uds. de SKUs desconocidos ignoradas`);
        } else {
